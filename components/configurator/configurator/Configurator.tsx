...
const SIZES: SizeOption[] = [
    ...
    { id: "xl", label: "200 x 100 cm", widthCm: 200, depthCm: 100, priceMod: 350 },
];
...
export function Configurator() {
    ...
    const totalPrice = BASE_PRICE + decor.priceMod + frame.priceMod + size.priceMod;
    ...
    return (
        <div className="w-full flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
            ...
            >
                {/* Background Grid */}
                <div className="absolute inset-0 opacity-10 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:40px_40px] [transform-style:preserve-3d] [perspective:1000px]"></div>
            ...
            <div className="w-full lg:w-1/3 space-y-10">
                <div>
                    ...
                    <p className="text-neutral-500 font-light">Ergonomic individual desk system</p>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {SIZES.map((opt) => (
                                <button
                                    key={opt.id}
                                    onClick={() => setSize(opt)}
                                    className={`px-4 py-3 border text-sm transition-all duration-300 ${size.id === opt.id
                        ...
                        <div className="flex flex-wrap gap-3">
                            {DECORS.map((opt) => (
                                <button
                                    ...
                                    className={`w-14 h-14 relative border transition-all duration-300 ${decor.id === opt.id ? "border-neutral-900 scale-110 shadow-lg" : "border-neutral-200 hover:border-neutral-400"
                                        }`}
                        ...
                        <div className="flex flex-wrap gap-3">
                            {FRAMES.map((opt) => (
                                <button
                                    ...
                                    className={`w-14 h-14 relative border transition-all duration-300 ${frame.id === opt.id ? "border-neutral-900 scale-110 shadow-lg" : "border-neutral-200 hover:border-neutral-400"
                                        }`}
                                    style={{ backgroundColor: opt.hex }}
                                    title={opt.name}
                                >
